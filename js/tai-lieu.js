// ============================================================
// tai-lieu.js — Tài liệu page logic for HOUSE WORK platform
// ============================================================

// ── 1. AUTH GUARD — handled by auth.js ──────────────────────
// initAuth() được gọi trong DOMContentLoaded bên dưới
...
function openFile(url) {
  ...
  const fileName = url.split('/').pop()
      .replace(/-/g, ' ').replace(/_/g, ' ').replace(/\.pdf$/i, '');
  document.getElementById('modalFileName').textContent = fileName;

  // Reset iframe trước (tránh flash nội dung cũ)
  ...
  // Load file sau 1 frame để modal animation chạy trước
  requestAnimationFrame(() => { viewer.src = url; });

  modal.onclick = (e) => { if (e.target === modal) closeFileModal(); };
}
